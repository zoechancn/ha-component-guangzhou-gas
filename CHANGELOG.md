...
### Changed
- 🎨 **添加集成图标**
  - ✅ `manifest.json` 添加 `"icon": "mdi:gas-cylinder"`
  - ✅ 集成现在在 HA 界面显示燃气罐图标

### Fixed
- 🐛 **修复用气地址状态传感器无图标问题**
  - ❌ v1.1.9 中用气地址状态传感器没有图标
  - ✅ 原因：`mdi:home-check` 不是有效的 MDI 图标
  - ✅ 修复：改为 `mdi:home-check-outline`
  - ✅ 修改文件：`sensor.py` 第1033行

- 🐛 **添加传感器调试日志**
  - ✅ 为问题传感器添加 `_LOGGER.debug()` 调用
  - ✅ 便于排查传感器不可用问题
  - ✅ 涉及传感器：安检日期、点火日期、保险截止日期、保险失效日期、最近充值时间
  - ✅ 修改文件：`sensor.py`
